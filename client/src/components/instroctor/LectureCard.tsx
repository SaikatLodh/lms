...
import { MonitorStop, SquarePen, Trash } from "lucide-react";
import { motion } from "motion/react";
import { InstructorLecture } from "@/types";
import { useDebounce } from "use-debounce";
import { Input } from "../ui/input";

const LectureCard = ({ courseId }: { courseId: string }) => {
  const { data, isLoading } = useGetLecture(courseId);
  ...
  return (
    <>
      <div className="md:flex items-center md:justify-between justify-evenly md:flex-nowrap flex-wrap">
        <h2 className="text-2xl md:text-5xl font-bold text-neutral-800 dark:text-neutral-200 font-sans mt-10">
          Lectures
        </h2>
        ...
        <Input
          ...
          className="md:w-[300px] w-full border border-black rounded-md p-2 md:mt-0 mt-3"
      ...
      {filteredSerch && filteredSerch.length === 0 && (
        <div className="w-full flex items-center justify-center gap-2">
          <div className="flex flex-col items-center justify-center">
            <MonitorStop width={60} height={60} />
            <h5 className="text-2xl font-bold">No Lectures Found</h5>
          </div>
        </div>
      )}
      {filteredSerch && filteredSerch.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-8 gap-0 w-full md:mt-0 mt-5">
          {filteredSerch.map((card) => (
            <Card key={card._id} card={card} courseId={courseId} />
          ))}
        </div>
      )}
    </>
...
const Card = ({
  ...
  return (
    <>
      <div className="w-full relative rounded-3xl overflow-hidden bg-gradient-to-r from-[#1D2235] to-[#121318] p-5 md:my-10 my-4 ">
        <Rays />
        <Beams />
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-2 md:flex-nowrap flex-wrap">
            <Link
              href={`/instructor/lecture/update/${card._id}`}
              className="w-full"
            >
            ...
            <button
              disabled={isPending}
              onClick={() => mutate({ courseId, lectureId: card._id })}
