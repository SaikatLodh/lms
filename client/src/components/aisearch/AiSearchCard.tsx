...
const AiSearchCard = ({ card }: { card: Aiearch }) => {
  ...
  const isCart = data && data.find((item) => item.courses._id === card._id);
  const isWishList =
    wishlist && wishlist.find((item) => item.courses._id === card._id);
  const { user, isAuthenticated } = useSelector(
    (state: RootState) => state.auth
  );
  const isPerchased = card.students.find((student) => student === user?._id);
  return (
    <>
      <div className="w-full relative rounded-3xl overflow-hidden max-w-md bg-gradient-to-r from-[#1D2235] to-[#121318] p-5 h-fit">
        <Rays />
        <Beams />
        <div className="relative z-10">
          ...
          {user && isAuthenticated ? (
            <>
              {isPerchased ? (
                <Link href={`/user/progress/${card._id}`}>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="relative inline-flex h-10 overflow-hidden rounded-full p-[2px] focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-gray-50"
                  >
                    <span className="absolute  inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#a2aeff_0%,#3749be_50%,#a2aeff_100%)] dark:bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)]" />
                    <span className="flex items-center gap-2  h-full w-full cursor-pointer  justify-center rounded-full dark:bg-[#070e41] bg-[#070E41] px-8 py-1 text-sm font-medium dark:text-gray-50 text-white backdrop-blur-3xl">
                  ...
                  <button
                    ...
                    disabled={
                      isPendingWishlist || card.instructorId === user._id
                    }
                    className="relative inline-flex h-10 overflow-hidden rounded-full p-[2px] focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-gray-50"
                  >
                    <span className="absolute  inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#a2aeff_0%,#3749be_50%,#a2aeff_100%)] dark:bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)]" />
