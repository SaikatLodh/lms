...
const CourseCard = ({ card }: { card: UserCourse }) => {
  ...
  return (
    <>
      <div className="w-full relative rounded-3xl overflow-hidden bg-gradient-to-r from-[#1D2235] to-[#121318] p-5 md:h-fit h-full">
        <Rays />
        <Beams />
        <div className="relative z-10">
          ...
          {user && isAuthenticated ? (
            <>
              {isPerchased ? (
                <Link href={`/user/progress/${card?._id}/${card?.lectures[0]}`}>
